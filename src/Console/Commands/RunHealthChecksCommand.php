...
namespace Cxj\LookingGlass\Console\Commands;

use Cxj\LookingGlass\Domain\HealthCheck;
use Exception;
...
{
    protected $signature = 'lg:check {--do-not-store-results} {--no-notification} {--fail-command-on-failing-check}';
        ...
        return app(Health::class)
            ...
            ->map(function (Check $check): Result {
                return $check->shouldRun()
                    ? $this->runCheck($check)
                    : (new Result(Status::skipped()))->check($check)->endedAt(
                        now()
                    );
            });
    }
